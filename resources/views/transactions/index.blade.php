...
@forelse($items as $item)<tr><td data-label="Tarih">{{ $item->occurred_on->format('d.m.Y') }}</td><td data-label="Tür"><span class="badge {{ $item->type }}">{{ $item->type === 'income' ? 'Gelir' : 'Gider' }}</span></td><td data-label="Kategori / Açıklama"><div class="transaction-cell-content"><strong>{{ $item->category }}</strong><small class="table-description" title="{{ $item->description }}">{{ \Illuminate\Support\Str::limit($item->description, 100) }}</small>@if($item->document_path)<small><a href="{{ route('documents.transaction', $item) }}">Belgeyi indir</a></small>@endif</div></td><td data-label="Personel">{{ $item->creator->name }}</td><td data-label="Tutar" class="numeric {{ $item->type === 'income' ? 'positive' : 'negative' }}">{{ number_format($item->amount, 2, ',', '.') }} ₺</td><td data-label="Durum"><span class="lock">🔒 Kilitli</span></td><td data-label="İşlem">@if(auth()->user()->hasPermission('transactions.manage'))<a href="{{ route('transactions.edit', $item) }}">Düzelt / Sil</a>@endif</td></tr>
