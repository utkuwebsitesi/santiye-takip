...
@section('content')
<div class="page-title"><div><h1>Tüm Hareketler</h1><p>Personelin girdiği gelir ve gider kayıtları</p></div><a class="btn primary" href="{{ route('transactions.create') }}">+ Yeni Hareket</a></div>
...
@forelse($items as $item)<tr><td>{{ $item->occurred_on->format('d.m.Y') }}</td><td><span class="badge {{ $item->type }}">{{ $item->type === 'income' ? 'Gelir' : 'Gider' }}</span></td><td><strong>{{ $item->category }}</strong><small>{{ $item->description }}</small>@if($item->document_path)<small><a href="{{ route('documents.transaction', $item) }}">Belgeyi indir</a></small>@endif</td><td>{{ $item->creator->name }}</td><td class="{{ $item->type === 'income' ? 'positive' : 'negative' }}">{{ number_format($item->amount, 2, ',', '.') }} ₺</td><td><span class="lock">🔒 Kilitli</span></td><td>@if(auth()->user()->isAdmin())<a href="{{ route('transactions.edit', $item) }}">Düzelt / Sil</a>@endif</td></tr>
