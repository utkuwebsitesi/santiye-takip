...
@section('content')
<div class="page-title"><div><h1>Plaka Bazlı Yakıt Geçmişi</h1><p>Tanker stokundan araç ve makinelere yapılan ikmaller ile maliyetleri</p></div>@if(auth()->user()->hasPermission('fuel.create'))<a class="btn primary" href="{{ route('fuel.create') }}">+ Araç İkmali</a>@endif</div>
...
@forelse($items as $item)<tr><td>{{ $item->fuel_date->format('d.m.Y') }}<small>{{ substr((string) $item->fuel_time, 0, 5) }}</small></td><td><strong>{{ $item->vehicle->display_name }}</strong><small>{{ $item->station }}</small>@if($item->receipt_path)<small><a href="{{ route('documents.fuel', $item) }}">Fişi indir</a></small>@endif</td><td>{{ $item->tanker?->name ?? 'Eski kayıt' }}</td><td class="numeric">{{ number_format($item->liters, 3, ',', '.') }} L</td><td class="numeric">{{ number_format($item->unit_price, 4, ',', '.') }} ₺/L</td><td class="numeric negative">{{ number_format($item->total_amount, 2, ',', '.') }} ₺</td><td class="numeric">{{ $item->meter_value !== null ? number_format($item->meter_value, 1, ',', '.').' km' : '—' }}<small>{{ $item->operating_hours !== null ? number_format($item->operating_hours, 1, ',', '.').' saat' : '—' }}</small></td><td>{{ $item->creator->name }}</td><td>@if(auth()->user()->hasPermission('fuel.manage'))<a href="{{ route('fuel.edit', $item) }}">Düzelt / Sil</a>@endif</td></tr>
