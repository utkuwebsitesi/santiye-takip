...
<div class="dashboard-page poster-dashboard">
    <div class="page-title"><div><h1>Genel Durum</h1><p>Şantiyenizin finansı, yakıtı ve filosu tek ekranda.</p></div><a class="btn primary" href="{{ route('transactions.create') }}">+ Yeni Hareket</a></div>
    ...
    <section class="poster-dashboard-grid">
        <article class="poster-card poster-tanker-card">
            <div class="poster-card-head"><div><h2>Tanker Stok Durumu</h2><small>Güncel stok ve dağılım</small></div><a href="{{ route('tankers.index') }}">Tüm stoklar</a></div>
        ...
        <article class="poster-card poster-fleet-card">
            <div class="poster-card-head"><div><h2>Araç &amp; Makine Yakıt Takibi</h2><small>Son dağıtımlar</small></div><a href="{{ route('fuel.index') }}">Yakıt raporu</a></div>
        ...
        <article class="poster-card poster-maintenance-card">
            <div class="poster-card-head"><div><h2>Bakım Hatırlatmaları</h2><small>Yaklaşan ve geciken işlemler</small></div><a href="{{ route('maintenance.index') }}">Bakımı gör</a></div>
    ...
    <section class="card"><div class="card-head"><h2>Son Kasa Hareketleri</h2><a href="{{ route('transactions.index') }}">Tümünü Gör</a></div>
        <div class="table-wrap"><table><thead><tr><th>Tarih</th><th>Açıklama</th><th>Personel</th><th>Tutar</th></tr></thead><tbody>
        @forelse($recentTransactions as $item)<tr><td>{{ $item->occurred_on->format('d.m.Y') }}</td><td>{{ $item->description }}<small>{{ $item->category }}</small></td><td>{{ $item->creator->name }}</td><td class="{{ $item->type === 'income' ? 'positive' : 'negative' }}">{{ $item->type === 'income' ? '+' : '-' }}{{ number_format($item->amount, 2, ',', '.') }} ₺</td></tr>
        @empty<tr><td colspan="4" class="empty">Henüz kasa hareketi yok.</td></tr>@endforelse
        </tbody></table></div>
    </section>
    <section class="card"><div class="card-head"><h2>Son Yakıt Kayıtları</h2><a href="{{ route('fuel.index') }}">Tümünü Gör</a></div>
